#!/usr/bin/python
"""
Ansible Module for promoting Content Lifecycle Project versions
...
---
module: clp_promote
short_description: Promote CLP version
description:
  - Promoting Content Lifecycle Project version
author:
...
options:
  project_label:
    ...
    type: str
  environment_label:
    description: Label of the environment to push to the next environment
    required: True
    type: str
'''
...
def _promote_clp(module, api_instance):
    """
    Promotes a Content Lifecycle Project
    """
    try:
        api_instance.promote_clp(
            module.params.get('project_label'),
            module.params.get('environment_label')
        )
        module.exit_json(changed=True)
    except SSLCertVerificationError:
        module.fail_json(msg="Failed to verify SSL certificate")
    except EmptySetException as err:
        module.fail_json(msg=f"Exception when calling UyuniAPI->promote_clp: {err}")
...
def main():
    ...
    argument_spec = dict(
        ...
        environment_label=dict(type='str', required=True)
    )
    ...
    _promote_clp(module, api_instance)
