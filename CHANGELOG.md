...
- **Breaking change**: removed Ansible role `uyuni.storage` as it's not needed anymore for containerized setups
...
- splitted former `uyuni` Ansible role in `uyuni.storage` and `uyuni.server` roles
